...
function getSmartPredictionAdjustment(type, prediction, patterns) {
  ...
  // Nếu gãy 2 tay (-2), bật chế độ đảo kết quả.
  // Nếu đảo kết quả sai (trở thành gãy 3 tay tức là -3), đảo tiếp tay thứ 2.
  // Nếu đảo tay 2 sai tiếp (gãy 4 tay, <= -4), quay về phân tích như ban đầu (bỏ qua đảo).
...
function calculateAdvancedPrediction(data, type) {
  ...
  // Điều chỉnh theo lịch sử thắng/thua (Nâng cấp đồng bộ với chống gãy)
  const streakInfo = learningData[type].streakAnalysis;
  if (streakInfo.currentStreak === -2 || streakInfo.currentStreak === -3) {
    // Nếu đang thua 2-3 tay, tăng điểm cho bên ngược lại với dự đoán hiện tại để hỗ trợ thuật toán bẻ
...
app.get('/lc79-hu', async (req, res) => {
  try {
    ...
    // --- TẠO CHUỖI PATTERN HIỂN THỊ KIỂU MỚI (VIP) ---
    // Trích xuất 15 kết quả gần nhất để làm mô phỏng chuỗi (t/x)
    const last15 = data.slice(0, 15).map(d => d.Ket_qua === 'Tài' ? 't' : 'x').reverse().join('');
    // Lấy nguyên nhân ưu tiên hàng đầu, nếu không có lấy mặc định
    const topFactor = result.factors.length > 0 ? result.factors[0] : 'Cầu Tự Nhiên';
    // Xây dựng chuỗi string VIP (viết hoa kết quả dự kiến)
    const patternStr = `[VIP CẦU: ${topFactor}] [CHUẨN] - Dự kiến: Cân bằng (${last15})`;

    // Map dữ liệu để trả về định dạng Uppercase chuẩn không dấu nếu có
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('  - Định dạng kết quả API chuẩn JSON VIP kiểu mới');
  console.log('  - Chống gãy 3 tay thông minh (Tự động đảo nếu gãy 2)');
  console.log('  - Thu gọn hiển thị lịch sử API /lichsu');
